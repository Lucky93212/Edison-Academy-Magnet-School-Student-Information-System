...
def attendance(request):
    # get attendance file and store using FileSystemStorage
    if request.method == 'POST':
        uploaded_file = request.FILES['file']
        fs = FileSystemStorage()
        fs.save("students.csv", uploaded_file)
        with open('media/students.csv', 'r') as file:
            reader = csv.reader(file)
            post = NewsAndEvents.objects.get(pk=request.POST.get('id'))
            for row in reader:
                username=(row[1].lower() + row[0][0].lower())
                student = Student.objects.get(student__username=username)
                student.points += post.points
                student.save()
        
        os.remove('media/students.csv')
        return redirect('home')
